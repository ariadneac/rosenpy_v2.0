...
def ru_weights_ptrbf(module, rows, cols, i=0):
    ...
    """
    # Generate real and imaginary parts sampled from a uniform distribution
    real = module.random.rand(rows, cols).astype(module.float32) 
    imag = module.random.rand(rows, cols).astype(module.float32) 
    
    # Scale the elements to have a unitary norm
    # by taking the square root of the exponential function of 2 divided by 4 times the product of rows and cols
...
def opt_ptrbf_weights(module, rows, cols, i=0):
    ...
    # Scale the elements for optimization
